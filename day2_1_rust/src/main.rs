...
fn checksum(spreadsheet_string: &str) -> i32 {
    let lines = spreadsheet_string.lines();
    let line_checksums = lines.map(|l| line_checksum(l));

    line_checksums.fold(0, |sum, line_sum| sum + line_sum)
}

fn line_checksum(line: &str) -> i32 {
    let mut numbers = line.split_whitespace().map(|n_str| n_str.parse::<i32>().unwrap());
    let first_n = numbers.next().unwrap();
    let mut largest = first_n;
    let mut smallest = first_n;
